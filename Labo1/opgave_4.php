...
/**
 * Lab 01, Exercise 4 — Start from this version
 * Companies
 * @author <Janis De Sutter>
 */

$companies =
    [['name' => 'Lunch Garden', 'address' => 'Avenue des Olympiades 2', 'zip' => 1140, 'city' => 'Bruxelles', 'activity' => 'Restaurants and mobile food service activities', 'vat' => 'BE0447668559'],
...
$a = 1;
$a = $argv[1]; //Eerste parameter want de "0de" is de naam van het bestand.
//show();
//showByName();
function show()
{
...
function showByName()
{
    global $companies;
    global $a;
    $key = array_search($a, array_column($companies, 'name'));
    var_dump($companies[$key]);
}
...
function findBySearchString()

{
    global $companies;
    global $a;

    $name = array_column($companies, 'name'); //maakt een apparte array van een kollom

    foreach ($name as $key => $value) {
        $bool = strposa($value, $a); //check ofdat de meegegeven substring er in voor komt en returnt true als dat zo is
